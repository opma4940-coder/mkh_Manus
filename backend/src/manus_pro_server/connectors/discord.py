# ═══════════════════════════════════════════════════════════════════════════════
# Discord Connector - موصل ديسكورد (مُنفّذ بالكامل)
# ═══════════════════════════════════════════════════════════════════════════════
...
class DiscordConnector(OAuthConnector):
    ...
    async def fetch(self, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        try:
            ...
            logger.error(f"Discord fetch failed: {e}")
            return []

    def get_authorization_url(self, state: Optional[str] = None) -> str:
        client_id = self.config.get("client_id")
        redirect_uri = self.config.get("redirect_uri")
        return f"https://discord.com/api/oauth2/authorize?client_id={client_id}&redirect_uri={redirect_uri}&response_type=code&scope=identify&state={state}"
