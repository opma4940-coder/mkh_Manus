# ═══════════════════════════════════════════════════════════════════════════════
# Snapchat Connector - موصل سناب شات (مُنفّذ بالكامل)
# ═══════════════════════════════════════════════════════════════════════════════
...
class SnapchatConnector(OAuthConnector):
    ...
    async def connect(self) -> bool:
        try:
            ...
            logger.error(f"Snapchat connection failed: {e}")
            return False
    ...
    async def send(self, payload: Dict[str, Any], attachments: Optional[List[str]] = None) -> Dict[str, Any]:
        try:
            ...
            logger.error(f"Snapchat send failed: {e}")
    ...
    async def fetch(self, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        try:
            ...
            logger.error(f"Snapchat fetch failed: {e}")
            return []

    def get_authorization_url(self, state: Optional[str] = None) -> str:
        client_id = self.config.get("client_id")
        redirect_uri = self.config.get("redirect_uri")
        return f"https://accounts.snapchat.com/login/oauth2/authorize?client_id={client_id}&redirect_uri={redirect_uri}&response_type=code&scope=snapchat-marketing-api&state={state}"
